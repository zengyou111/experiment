...
# 训练函数
def train_contrastive_model(encoded_samples, val_encoded_samples, contrastive_model, epochs=projectionHeadEpoch, temperature=Temperature, patience=3, exp_name=""):
    print(f"\n{'='*50}")
...
def evaluate_clones(samples, model, contrastive_model):
    ...
    for sample in samples:
        anchor = sample['code_embedding'].to(device)  # 保持 [1, embedding_dim]
...
results['Full_Conba'] = {'train_losses': losses, 'val_losses': val_losses, 'acc': acc, 'f1': f1, 'recall': recall, 'precision': precision}
...
for name, layer_class, desc in ablation_configs:
    ...
    acc, f1, recall, precision = evaluate_clones(test_samples, model, contrastive_model)
    results[name] = {'train_losses': losses, 'val_losses': val_losses, 'acc': acc, 'f1': f1, 'recall': recall, 'precision': precision}
...
colors = {
    ...
    'MLP': 'orange', 'GRU': 'brown', 'CNN': 'cyan', 'LSTM': 'pink', 'BiLSTM': 'gray', 'RNN': 'magenta', 'ResNet': 'yellow'
}
...
for metric in metrics:
    ...
    plt.plot(list(results.keys()), values, marker='o', label=metric, color=colors['Full_Conba'])
    plt.xlabel('Model')
