...
# %%
#### 参数配置
projectionHeadEpoch=1000                                  #### 对比学习最大epoch
beforeProjectionEpoch=1000                                #### 初始emb+MOE的最大epoch 
afterProjectionEpoch=1000                                 #### projection(emb)+MOE最大epoch
projectionHeadPatience=3                                 #### 对比学习最大patience
beforeProjectionPatience=3                               #### emb+Moe的最大patience
afterProjectionPatience=3                                #### projection(emb)+MOE最大epoch
index=5                                                  #### 预训练编码器的index
lang=3                                                   #### 语言index
size=3000                                                #### 数据集批量
proLearningRate=1e-7                                     #### 对比学习的学习率
beforeProjectionLearningRate=1e-7                        #### 初始emd+MOE的学习率
afterProjectionLearningRate=1e-7                        #### projection(emb)+MOE的最大epoch
...
optimizer = optim.AdamW(contrastive_model.parameters(), lr=proLearningRate)
...
mlp_optimizer_contrastive = optim.AdamW(mlp_model_contrastive.parameters(), lr=afterProjectionEpoch)
...
def plot_control_weights(control_weights_history, num_features_to_plot=20):
    ...
    plt.tight_layout()
    plt.savefig(time.time().__str__()+'Control Weights Changes Over Epochs'+".png")
    plt.show()
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Mixnet Loss Convergence')
plt.legend(loc='upper right')  # 将图例放在左上角
plt.tight_layout()
plt.savefig(time.time().__str__() + 'Mixnet Learning Loss Convergence' + ".png")
plt.show()
...
plt.ylabel('Loss')
plt.legend(loc='upper right')  # 将图例放在左上角
plt.title('Initial Embedding + MixClassiferConvergence')
plt.tight_layout()
plt.savefig(time.time().__str__()+'Initial Embedding + Mix Loss Convergence'+".png")
plt.show()
...
plt.ylabel('Loss')
plt.legend(loc='upper right')  # 将图例放在左上角
plt.title('MixNet Embedding + MixClassifier Loss Convergence')
plt.tight_layout()
plt.savefig(time.time().__str__()+'Mixcon Embedding +  MixClassifier Loss Convergence'+".png")
plt.show()
...
plot_tsne(test_samples, contrastive_model, title='T-SNE of MixNet Embeddings')
